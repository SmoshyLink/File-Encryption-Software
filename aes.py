# AES #

import os
from hashlib import pbkdf2_hmac
from hmac import new as new_hmac, compare_digest

AES_KEY_SIZE = 16
HMAC_KEY_SIZE = 16
IV_SIZE = 16

SALT_SIZE = 16
HMAC_SIZE = 32

sbox = (
    ,
)

inverse_sbox = (
    ,
)


def add_round_key(block, key):
    for i in range(4):
        for j in range(4):
            block[i][j] = (block[i][j]) ^ (key[i][j])


def substitute_bytes(block):
    for i in range(4):
        for j in range(4):
            block[i][j] = sbox[block[i][j]]

def inverse_sbox(block):
    for i in range(4):
        for j in range(4):
            block[i][j] = inverse_sbox[block[i][j]]


def shift_rows(block):
    block[0][1], block[1][1], block[2][1], block[3][1] = block[1][1], block[2][1], block[3][1], block[0][1]
    block[0][2], block[1][2], block[2][2], block[3][2] = block[2][2], block[3][2], block[0][2], block[1][2]
    block[0][3], block[1][3], block[2][3], block[3][3] = block[3][3], block[0][3], block[1][3], block[2][3]

def inverse_shift_rows(block):
    block[0][1], block[1][1], block[2][1], block[3][1] = block[3][1], block[0][1], block[1][1], block[2][1]
    block[0][2], block[1][2], block[2][2], block[3][2] = block[2][2], block[3][2], block[0][2], block[1][2]
    block[0][3], block[1][3], block[2][3], block[3][3] = block[1][3], block[2][3], block[3][3], block[0][3]


galois = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)

def mix_single_column(a):
    xor = a[0] ^ a[1] ^ a[2] ^ a[3]
    temp = a[0]
    a[0] ^= xor ^ galois(a[0] ^ a[1])
    a[1] ^= xor ^ galois(a[1] ^ a[2])
    a[2] ^= xor ^ galois(a[2] ^ a[3])
    a[3] ^= xor ^ galois(a[3] ^ temp)

def mix_columns(block):
    for i in range(4):
        mix_single_column(block[i])

def inv_mix_columns(block):
    for i in range(4):
        u = galois(galois(block[i][0] ^ block[i][2]))
        v = galois(galois(block[i][1] ^ block[i][3]))
        block[i][0] ^= u
        block[i][1] ^= v
        block[i][2] ^= u
        block[i][3] ^= v

    mix_columns(block)


rcon = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)


def bytesToMatrix(text):
    return [list(text[i: i + 4]) for i in range(0, len(text), 4)]

def matrixToBytes(matrix):
    return bytes(sum(matrix, []))

def xor(a, b):
    return bytes(i ^ j for i, j in zip(a, b))

def inc_bytes(a):
    out = list(a)
    for i in reversed(range(len(out))):
        if out[i] == 0xFF:
            out[i] = 0
        else:
            out[i] += 1
            break
    return bytes(out)

def pad(plaintext):
    padding_len = 16 - (len(plaintext) % 16)
    padding = bytes([padding_len] * padding_len)
    return plaintext + padding

def unpad(plaintext):
    padding_len = plaintext[-1]
    message, padding = plaintext[:-padding_len], plaintext[-padding_len:]
    return message

def split_blocks(message, block_size=16, require_padding=True):
    return [message[i: i + 16] for i in range(0, len(message), block_size)]


class AES:

    keysizeRounds = {16: 10, 24: 12, 32: 14}

    def __init__(self, headKey):
        self.n_rounds = AES.keysizeRounds[len(headKey)]
        self._key_matrices = self._expand_key(headKey)

    def _expand_key(self, headKey):
        key_columns = bytesToMatrix(headKey)
        iteration_size = len(headKey) // 4

        columns_per_iteration = len(key_columns)
        i = 1
        while len(key_columns) < (self.n_rounds + 1) * 4:
            keyPart = list(key_columns[-1])

            if len(key_columns) % iteration_size == 0:
                keyPart.append(keyPart.pop(0))
                keyPart = [sbox[b] for b in keyPart]
                keyPart[0] ^= rcon[i]
                i += 1
            elif len(headKey) == 32 and len(key_columns) % iteration_size == 4:
                keyPart = [sbox[b] for b in keyPart]

            keyPart = xor(keyPart, key_columns[-iteration_size])
            key_columns.append(keyPart)

        return [key_columns[4 * i : 4 * (i + 1)] for i in range(len(key_columns) // 4)]

    def encrypt_state(self, plaintext):
        plain_state = bytesToMatrix(plaintext)

        add_round_key(plain_state, self._key_matrices[0])

        for i in range(1, self.n_rounds):
            substitute_bytes(plain_state)
            shift_rows(plain_state)
            mix_columns(plain_state)
            add_round_key(plain_state, self._key_matrices[i])

        substitute_bytes(plain_state)
        shift_rows(plain_state)
        add_round_key(plain_state, self._key_matrices[-1])

        return matrixToBytes(plain_state)

    def decrypt_state(self, ciphertext):
        cipher_state = bytesToMatrix(ciphertext)

        add_round_key(cipher_state, self._key_matrices[-1])
        inverse_shift_rows(cipher_state)
        inverse_sbox(cipher_state)

        for i in range(self.n_rounds - 1, 0, -1):
            add_round_key(cipher_state, self._key_matrices[i])
            inv_mix_columns(cipher_state)
            inverse_shift_rows(cipher_state)
            inverse_sbox(cipher_state)

        add_round_key(cipher_state, self._key_matrices[0])

        return matrixToBytes(cipher_state)



def encrypt(key, plaintext, workload=100000):
    if isinstance(key, str):
        key = key.encode('utf-8')
    if isinstance(plaintext, str):
        plaintext = plaintext.encode('utf-8')

    ciphertext = AES(key).encrypt_state(plaintext)

    return ciphertext


def decrypt(key, ciphertext, workload=100000):
    if isinstance(key, str):
        key = key.encode('utf-8')

    return AES(key).decrypt_state(ciphertext)